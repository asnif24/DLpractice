...
def plotAndSaveRealPic(step):
    plot()
    plt.savefig(file_dir+str(step)+'_real_Dreal_.png', bbox_inches='tight', pad_inches = 0)

def plotAndSaveFakePic(step):
    plot()
    plt.savefig(file_dir+str(step)+'_fake_Dfake_.png', bbox_inches='tight', pad_inches = 0)




layer_e1 = layer(28*23*72, 1024)
layer_e2 = layer(1024, h_dim)

layer_d1 = layer(h_dim, 1024)
layer_d2 = layer(1024, 28*23*72)

layer_g1 = layer(z_dim, 1024)
...
x_g = tf.placeholder(tf.float32, shape = [None, z_dim])
...
for step in range(800001):
    ...
    if step%200==0:
        d_loss_train, g_loss_train, M_global_train = sess.run([d_loss, g_loss, M_global], feed_dict=
                            {x_d: batch_x, x_g: sample_Z(batch_size, z_dim)})
        ...
        for i in range(10):
            ...
            plt.axis('off')
        plt.subplots_adjust(wspace=0, hspace=0)
        plt.savefig('began_pic/pic0208/'+str(step)+'_real_Dreal_.png', bbox_inches='tight', pad_inches = 0)
        ...
        for i in range(10):
            ...
            plt.axis('off')
        plt.subplots_adjust(wspace=0, hspace=0)
        plt.savefig('began_pic/pic0208/'+str(step)+'_fake_Dfake.png', bbox_inches='tight', pad_inches = 0)
